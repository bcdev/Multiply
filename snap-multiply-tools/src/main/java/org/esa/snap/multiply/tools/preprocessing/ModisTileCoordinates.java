 ...
 * Taken from BEAM GlobAlbedo software.
 *
 * @author MarcoZ
 */
class ModisTileCoordinates {
    ...
    /**
     * Provides the Modis tile upper left coordinated from auxdata
     *
     */
    ...
    static ModisTileCoordinates getInstance() {
        return Holder.instance;
    }

    double getUpperLeftX(int tileIndex) {
        return tiles.get(tileIndex).x;
    }
    double getUpperLeftY(int tileIndex) {
        return tiles.get(tileIndex).y;
    }

    int findTileIndex(String tileName) {
    ...
    private List<TileCoordinate> loadAuxData() {
        ...
        List<TileCoordinate> tiles = new ArrayList<>(records.size());
